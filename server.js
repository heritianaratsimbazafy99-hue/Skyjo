...
function computeRound(state, rawScores, closerId) {
  const adjustedScores = { ...rawScores };
  let closerPenaltyApplied = false;

  if (state.doubleCloserPenalty && closerId) {
    const closerScore = rawScores[closerId];
    const hasEqualOrLowerOpponent = state.players.some((player) => player.id !== closerId && rawScores[player.id] <= closerScore);
